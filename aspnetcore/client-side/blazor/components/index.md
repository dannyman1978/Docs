...
The following pair of Blazor components demonstrates how Blazor takes advantage of features in Razor, C#, and HTML. These features are described further later in this topic.
...
A Razor file for Blazor specifies [Razor directives](xref:mvc/views/razor#directives) at the top of the file using the familiar `@` symbol to distinguish Razor code from HTML markup. Razor directives supported by Blazor are shown in the table.
...
| [@page](xref:mvc/razor-pages/index#razor-pages) | Specifies that the component should handle requests directly. The `@page` directive can be specified with a route and optional parameters. Note that unlike Razor Pages, the `@page` directive doesn't need to be the first directive at the top of the file. |
...
The following component inherits a base class, `BlazorRocksBase`, to provide its properties and methods:
...
One-way databinding is accomplished by prefixing the `@` symbol to any available property. The following example binds the `HeadingFontStyle` property to a CSS property value and `HeadingText` to the content of the `<h1>` element:
...
Two-way databinding to both components and DOM elements is accomplished with the `bind` attribute. The following example binds the `ItalicsCheck` property to the checkbox:
...
Updating the checkbox in the UI updates the property. Updating the property value in code updates the checkbox in the UI.

**Format strings**

Two-way databinding works with DateTime format stings (but not other format expressions):
...
The following parent component uses `ChildComponent` and binds the value in `CurrentValue` to the child component's `Value` property:

```cshtml
<ChildComponent bind-Value="@CurrentValue" />
...
`bind` can be specified with any value attribute name and change handler attribute name:
...
This binds the current value of `SomeExpression` to `myvalue` and a change handler lambda to `myevent`.
...
Blazor components can implement [IDisposable](/dotnet/api/system.idisposable). If they do, the router disposes the component when the user navigates away from the component. If you implement a component purely in C#, you can implement `IDisposable`. If you use a Blazor template, you can use `@implements IDisposable`:
...
Routing in Blazor is achieved by providing a route template to each accessible component. Route templates are converted into [RouteAttribute](/dotnet/api/microsoft.aspnetcore.mvc.routeattribute) implementations. Multiple route templates can be applied to a component. The following component responds to requests for `/BlazorRoute` and `/DifferentBlazorRoute`:
...
Not all route parameter constraints are supported at this time. Optional parameters aren't currently supported, so two `@page` directives are applied in the example. The first permits navigation to the component without a parameter. The second `@page` directive takes the `{adjective}` route parameter and assigns the value to the `Adjective` property.
